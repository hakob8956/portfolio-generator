...
function setData(data) {
  ...
  if (data.hideDesignedBy) hideFooter();
}
...
function addContacts(personal) {
  if (personal.email != undefined && personal.email != "")
    append(
      ...
      HTMLEmail.replaceAll("%data%", personal.email)
    );
  if (personal.phone != undefined && personal.phone != "")
    append(
      document.getElementById("contacts"),
      HTMLPhone.replaceAll("%data%", personal.phone)
    );
  if (personal.location != undefined && personal.location != "")
    append(
      document.getElementById("contacts"),
      HTMLLocation.replaceAll("%data%", personal.location)
    );
  let websitesBlock = "";
  let htmlPersonalWebsites = [];

  if (personal.websites != undefined && personal.websites.length > 0) {
    personal.websites.forEach((element) => {
      let htmlPersonalWebsite = HTMLPersonalWebsite.replace(
        "%url%",
        element.url
      );
      htmlPersonalWebsite = htmlPersonalWebsite.replace(
        "%label%",
        element.label
      );
      ...
      websitesBlock += htmlPersonalWebsite;
    });
    append(
      document.getElementById("contacts"),
      HTMLPersonalWebsiteBlock.replace("%data%", websitesBlock)
    );
  }
}
...
function addProjectSections(data) {
  ...
  for (var i in projectSections) {
    var projectSectionHtml = HTMLProjectsStart.replace(
      "%sectionTitle%",
      projectSections[i].title
    );
...
function addProjects(projects, domProjects) {
  ...
  for (var i in projects) {
    ...
    if (
      projects[i].description != undefined &&
      projects[i].description.length != 0
    ) {
      if (projects[i].description instanceof Array) {
        projectHtml = addProjectBulletDescription(
          projectHtml,
          projects[i].description
        );
      } else
...
function addProjectBulletDescription(projectHtml, description) {
  ...
  for (var i in description) {
    listHtml += HTMLProjectBulletDescriptionItem.replace(
      "%data%",
      description[i]
    );
  }
...
function addAwards(data) {
  if (data.awards == undefined || data.awards.length == 0) return;
  ...
  for (var i in awards) {
    ...
    var awarderAndDate = HTMLAwarderAndDate.replace(
      "%awarder%",
      awards[i].awarder
    );
    ...
    if (awards[i].description != null && awards[i].description != "") {
      ...
      append(
        domAwards,
        HTMLAwardDetails.replace("%id%", divId).replace(
          "%data%",
...
function addVolunteer(data) {
  if (data.volunteer == undefined || data.volunteer.length == 0) return;
  ...
  for (var i in volunteer) {
    ...
    var volunteerDateAndLocation = HTMLEmploymentDateAndLocation.replace(
      "%date%",
      volunteer[i].dates === undefined ? "" : volunteer[i].dates
    );
    volunteerDateAndLocation = volunteerDateAndLocation.replace(
      "%location%",
      volunteer[i].location === undefined ? "" : volunteer[i].location
    );
    var organization = "";

    if (volunteer[i].website != undefined && volunteer[i].website != "") {
      organization = HTMLEmployer.replace(
        "%employer%",
        volunteer[i].organization
      );
      organization = organization.replace("%url%", volunteer[i].website);
    } else {
      organization = HTMLEmployerWithoutUrl.replace(
        "%employer%",
        volunteer[i].organization
      );
    }
    ...
    if (
      volunteer[i].highlights != undefined &&
      volunteer[i].highlights instanceof Array &&
      volunteer[i].highlights.length != 0
    ) {
      var unique = new Date().getMilliseconds();
      ...
      append(
        domVolunteer,
        HTMLWorkHighlightsTitle.replace("%targetId%", highlightId).replace(
          "%id%",
          divId
        )
      );
      ...
      append(
        highlightDiv,
        HTMLWorkHighlightsList.replace("%data%", highlightId)
      );
      var highlightList = document.getElementById(highlightId);
      for (var j in volunteer[i].highlights) {
        append(
          highlightList,
          HTMLWorkHighlightsListItem.replace(
            "%data%",
            volunteer[i].highlights[j]
          )
        );
      }
    }
...
function addWork(data) {
  if (data.work === undefined || data.work.length === 0) return;
  var work = data.work;
  ...
  for (var i in work) {
    ...
    var employmentDateAndLocation = HTMLEmploymentDateAndLocation.replace(
      "%date%",
      work[i].dates === undefined ? "" : work[i].dates
    );
    employmentDateAndLocation = employmentDateAndLocation.replace(
      "%location%",
      work[i].location === undefined ? "" : work[i].location
    );
    var employer = "";
    ...
    if (work[i].highlights != null && work[i].highlights.length != 0) {
      ...
      append(
        domWork,
        HTMLWorkHighlightsTitle.replace("%targetId%", divId).replace(
          "%id%",
          highlightId
        )
      );
      ...
      for (var j in work[i].highlights) {
        append(
          highlightList,
          HTMLWorkHighlightsListItem.replace("%data%", work[i].highlights[j])
        );
      }
    }
  }
...
function addSocialLinks(data) {
  if (
    data.personal.profiles === undefined ||
    data.personal.profiles.length === 0
  )
    return;
  ...
  for (var i in social) {
    var socialItem = HTMLSocialItem.replace(
      "%network%",
      social[i].network.toLowerCase()
    );
